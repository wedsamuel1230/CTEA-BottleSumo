...
// OLED I2C 引腳配置（使用 Wire1 實例，與 ADS1115 的 Wire 分離）
...
const unsigned long STREAM_SEND_INTERVAL = 50;    // 50ms 發送間隔 (20Hz)
unsigned long lastTCPCheck = 0;
const unsigned long TCP_CHECK_INTERVAL = 25;      // 25ms TCP檢查間隔 (40Hz)

// 多客戶端即時串流管理
const int MAX_STREAMING_CLIENTS = 6;               // 最大同時串流客戶端數
...
bool initWiFiAndStreamingServer() {
  ...
  Serial.printf("Streaming Rate: %dHz (Every %dms)\n", 1000/STREAM_SEND_INTERVAL, STREAM_SEND_INTERVAL);
  ...
  Serial.printf("🔄 Streaming Format: JSON @ %dHz\n", 1000/STREAM_SEND_INTERVAL);
...
void printSystemInfo() {
  ...
  if (wifiConnected) {
    ...
    Serial.printf("- Real-Time Streaming: Port %d @ %dHz\n", TCP_SERVER_PORT, 1000/STREAM_SEND_INTERVAL);
